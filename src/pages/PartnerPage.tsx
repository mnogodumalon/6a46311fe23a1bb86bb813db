...
export default function PartnerPage() {
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getPartner());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Partner['fields']) {
    await LivingAppsService.createPartnerEntry(fields);
    await loadData();
    setDialogOpen(false);
  }
  ...
  async function handleUpdate(fields: Partner['fields']) {
    ...
    await LivingAppsService.updatePartnerEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deletePartnerEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      title="Partner"
      subtitle={`${records.length} Partner im System`}
      action={
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; navigate(`/partner/${record.record_id}`); }}>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={11} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Partner. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
