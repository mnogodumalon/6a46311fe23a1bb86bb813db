...
import { VertraegeViewDialog } from '@/components/dialogs/VertraegeViewDialog';
import { PartnerDialog } from '@/components/dialogs/PartnerDialog';
import { PartnerViewDialog } from '@/components/dialogs/PartnerViewDialog';
import { AnsprechpartnerDialog } from '@/components/dialogs/AnsprechpartnerDialog';
import { AnsprechpartnerViewDialog } from '@/components/dialogs/AnsprechpartnerViewDialog';
...
const VERTRAEGE_FIELDS = [
  { key: 'vertragsbezeichnung', label: 'Vertragsbezeichnung', type: 'string/text' },
  { key: 'vertragsnummer', label: 'Vertragsnummer', type: 'string/text' },
  { key: 'kategorie', label: 'Kategorie', type: 'lookup/select', options: [{ key: 'dienstleistung', label: 'Dienstleistung' }, { key: 'liefervertrag', label: 'Liefervertrag' }, { key: 'lizenzvertrag', label: 'Lizenzvertrag' }, { key: 'mietvertrag', label: 'Mietvertrag' }, { key: 'wartungsvertrag', label: 'Wartungsvertrag' }, { key: 'rahmenvertrag', label: 'Rahmenvertrag' }, { key: 'kooperationsvertrag', label: 'Kooperationsvertrag' }, { key: 'sonstiges', label: 'Sonstiges' }] },
  { key: 'status', label: 'Status', type: 'lookup/select', options: [{ key: 'aktiv', label: 'Aktiv' }, { key: 'in_verhandlung', label: 'In Verhandlung' }, { key: 'ruhend', label: 'Ruhend' }, { key: 'gekuendigt', label: 'Gekündigt' }, { key: 'abgelaufen', label: 'Abgelaufen' }, { key: 'entwurf', label: 'Entwurf' }] },
  { key: 'partner', label: 'Partner', type: 'applookup/select', targetEntity: 'partner', targetAppId: 'PARTNER', displayField: 'firmenname' },
  { key: 'ansprechpartner', label: 'Ansprechpartner', type: 'applookup/select', targetEntity: 'ansprechpartner', targetAppId: 'ANSPRECHPARTNER', displayField: 'vorname' },
  { key: 'jahresvolumen', label: 'Jahresvolumen (€)', type: 'number' },
  { key: 'startdatum', label: 'Startdatum', type: 'date/date' },
  { key: 'enddatum', label: 'Enddatum', type: 'date/date' },
  { key: 'kuendigungsfrist', label: 'Kündigungsfrist', type: 'lookup/select', options: [{ key: 'zwei_wochen', label: '2 Wochen' }, { key: 'ein_monat', label: '1 Monat' }, { key: 'zwei_monate', label: '2 Monate' }, { key: 'drei_monate', label: '3 Monate' }, { key: 'sechs_monate', label: '6 Monate' }, { key: 'zwoelf_monate', label: '12 Monate' }, { key: 'keine', label: 'Keine' }] },
  { key: 'kuendigungsdatum', label: 'Kündigungsdatum', type: 'date/date' },
  { key: 'vertragsdokument', label: 'Vertragsdokument', type: 'file' },
  { key: 'notizen', label: 'Notizen', type: 'string/textarea' },
];
const PARTNER_FIELDS = [
  { key: 'firmenname', label: 'Firmenname', type: 'string/text' },
  { key: 'branche', label: 'Branche', type: 'lookup/select', options: [{ key: 'it_software', label: 'IT & Software' }, { key: 'finanzen_versicherung', label: 'Finanzen & Versicherung' }, { key: 'logistik_transport', label: 'Logistik & Transport' }, { key: 'handel_retail', label: 'Handel & Retail' }, { key: 'produktion_industrie', label: 'Produktion & Industrie' }, { key: 'beratung_dienstleistung', label: 'Beratung & Dienstleistung' }, { key: 'gesundheit_pharma', label: 'Gesundheit & Pharma' }, { key: 'medien_marketing', label: 'Medien & Marketing' }, { key: 'bau_immobilien', label: 'Bau & Immobilien' }, { key: 'sonstige', label: 'Sonstige' }] },
...
const ANSPRECHPARTNER_FIELDS = [
  ...
  { key: 'email_ap', label: 'E-Mail', type: 'string/email' },
  { key: 'telefon_ap', label: 'Telefon', type: 'string/tel' },
  { key: 'partner', label: 'Partner', type: 'applookup/select', targetEntity: 'partner', targetAppId: 'PARTNER', displayField: 'firmenname' },
];

const ENTITY_TABS = [
  { key: 'vertraege', label: 'Verträge', pascal: 'Vertraege' },
  { key: 'partner', label: 'Partner', pascal: 'Partner' },
  { key: 'ansprechpartner', label: 'Ansprechpartner', pascal: 'Ansprechpartner' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('vertraege');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'vertraege': new Set(),
    'partner': new Set(),
    'ansprechpartner': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'vertraege': return (data as any).vertraege as Vertraege[] ?? [];
      case 'partner': return (data as any).partner as Partner[] ?? [];
      case 'ansprechpartner': return (data as any).ansprechpartner as Ansprechpartner[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getLookupLists = useCallback((entity: EntityKey) => {
    ...
    switch (entity) {
      case 'vertraege':
        lists.partnerList = (data as any).partner ?? [];
        lists.ansprechpartnerList = (data as any).ansprechpartner ?? [];
        break;
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'vertraege' && fieldKey === 'partner') {
      const match = (lists.partnerList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.firmenname ?? '—';
    }
    if (entity === 'vertraege' && fieldKey === 'ansprechpartner') {
      const match = (lists.ansprechpartnerList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.vorname ?? '—';
    }
    if (entity === 'ansprechpartner' && fieldKey === 'partner') {
      const match = (lists.partnerList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.firmenname ?? '—';
    }
    return String(url);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'vertraege': return VERTRAEGE_FIELDS;
      case 'partner': return PARTNER_FIELDS;
      case 'ansprechpartner': return ANSPRECHPARTNER_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'vertraege': return {
        create: (fields: any) => LivingAppsService.createVertraegeEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateVertraegeEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteVertraegeEntry(id),
      };
      case 'partner': return {
        create: (fields: any) => LivingAppsService.createPartnerEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updatePartnerEntry(id, fields),
        remove: (id: string) => LivingAppsService.deletePartnerEntry(id),
      };
      case 'ansprechpartner': return {
        create: (fields: any) => LivingAppsService.createAnsprechpartnerEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateAnsprechpartnerEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteAnsprechpartnerEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'vertraege' || dialogState?.entity === 'vertraege') && (
        <VertraegeDialog
          open={createEntity === 'vertraege' || dialogState?.entity === 'vertraege'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'vertraege' ? handleUpdate : (fields: any) => handleCreate('vertraege', fields)}
          defaultValues={dialogState?.entity === 'vertraege' ? dialogState.record?.fields : undefined}
          partnerList={(data as any).partner ?? []}
          ansprechpartnerList={(data as any).ansprechpartner ?? []}
          enablePhotoScan={AI_PHOTO_SCAN['Vertraege']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Vertraege']}
        />
      )}
      {(createEntity === 'partner' || dialogState?.entity === 'partner') && (
        <PartnerDialog
          open={createEntity === 'partner' || dialogState?.entity === 'partner'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'partner' ? handleUpdate : (fields: any) => handleCreate('partner', fields)}
          defaultValues={dialogState?.entity === 'partner' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Partner']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Partner']}
        />
      )}
      {(createEntity === 'ansprechpartner' || dialogState?.entity === 'ansprechpartner') && (
        <AnsprechpartnerDialog
          open={createEntity === 'ansprechpartner' || dialogState?.entity === 'ansprechpartner'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'ansprechpartner' ? handleUpdate : (fields: any) => handleCreate('ansprechpartner', fields)}
          defaultValues={dialogState?.entity === 'ansprechpartner' ? dialogState.record?.fields : undefined}
          partnerList={(data as any).partner ?? []}
          enablePhotoScan={AI_PHOTO_SCAN['Ansprechpartner']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Ansprechpartner']}
        />
      )}
      {viewState?.entity === 'vertraege' && (
        <VertraegeViewDialog
          open={viewState?.entity === 'vertraege'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'vertraege', record: r }); }}
          partnerList={(data as any).partner ?? []}
          ansprechpartnerList={(data as any).ansprechpartner ?? []}
        />
      )}
      {viewState?.entity === 'partner' && (
        <PartnerViewDialog
          open={viewState?.entity === 'partner'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'partner', record: r }); }}
        />
      )}
      {viewState?.entity === 'ansprechpartner' && (
        <AnsprechpartnerViewDialog
          open={viewState?.entity === 'ansprechpartner'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'ansprechpartner', record: r }); }}
          partnerList={(data as any).partner ?? []}
        />
      )}
