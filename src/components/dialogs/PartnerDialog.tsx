...
import type { Partner, LookupValue } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Partner';
...
interface PartnerDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Partner['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: Omit<Partner['fields'], 'branche'> & {
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  branche: LOOKUP_OPTIONS['partner']?.['branche'] ?? [],
};
...
export function PartnerDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: PartnerDialogProps) {
  const [fields, setFields] = useState<Partial<Partner['fields']>>({});
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Partner['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'partner');
      await onSubmit(clean as Partner['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "firmenname": string | null, // Firmenname\n  "branche": LookupValue | null, // Branche (select one key: "it_software" | "finanzen_versicherung" | "logistik_transport" | "handel_retail" | "produktion_industrie" | "beratung_dienstleistung" | "gesundheit_pharma" | "medien_marketing" | "bau_immobilien" | "sonstige") mapping: it_software=IT & Software, finanzen_versicherung=Finanzen & Versicherung, logistik_transport=Logistik & Transport, handel_retail=Handel & Retail, produktion_industrie=Produktion & Industrie, beratung_dienstleistung=Beratung & Dienstleistung, gesundheit_pharma=Gesundheit & Pharma, medien_marketing=Medien & Marketing, bau_immobilien=Bau & Immobilien, sonstige=Sonstige\n  "strasse": string | null, // Straße\n  "hausnummer": string | null, // Hausnummer\n  "postleitzahl": string | null, // Postleitzahl\n  "ort": string | null, // Ort\n  "telefon": string | null, // Telefon\n  "email": string | null, // E-Mail\n  "webseite": string | null, // Webseite\n  "notizen": string | null, // Notizen\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Partner['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Partner bearbeiten' : 'Partner hinzufügen';
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'branche': (
      <div key="branche" className="space-y-1.5">
        ...
        >
          <SelectTrigger id="branche" className="max-sm:h-11"><SelectValue placeholder="z. B. IT & Software, Logistik & Transport" /></SelectTrigger>
          <SelectContent>
            <SelectItem value="none">—</SelectItem>
            <SelectItem value="it_software">IT & Software</SelectItem>
            <SelectItem value="finanzen_versicherung">Finanzen & Versicherung</SelectItem>
            <SelectItem value="logistik_transport">Logistik & Transport</SelectItem>
            <SelectItem value="handel_retail">Handel & Retail</SelectItem>
            <SelectItem value="produktion_industrie">Produktion & Industrie</SelectItem>
            <SelectItem value="beratung_dienstleistung">Beratung & Dienstleistung</SelectItem>
            <SelectItem value="gesundheit_pharma">Gesundheit & Pharma</SelectItem>
            <SelectItem value="medien_marketing">Medien & Marketing</SelectItem>
            <SelectItem value="bau_immobilien">Bau & Immobilien</SelectItem>
    ...
    'notizen': (
      <div key="notizen" className="space-y-1.5">
        ...
        <Textarea
          id="notizen"
          placeholder="Besonderheiten, Geschäftsbeziehung, Kontakt-Historie..."
  ...
  const FIELD_LABELS: Record<string, string> = {"firmenname": "Firmenname", "branche": "Branche", "strasse": "Straße", "hausnummer": "Hausnummer", "postleitzahl": "Postleitzahl", "ort": "Ort", "telefon": "Telefon", "email": "E-Mail", "webseite": "Webseite", "notizen": "Notizen"};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.PARTNER} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
