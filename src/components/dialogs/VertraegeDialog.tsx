...
import type { Vertraege, Partner, Ansprechpartner, LookupValue } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Vertraege';
...
interface VertraegeDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Vertraege['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: Omit<Vertraege['fields'], 'kategorie' | 'status' | 'kuendigungsfrist'> & {
    kategorie?: LookupValue | string;
    status?: LookupValue | string;
    kuendigungsfrist?: LookupValue | string;
  };
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  kategorie: LOOKUP_OPTIONS['vertraege']?.['kategorie'] ?? [],
  status: LOOKUP_OPTIONS['vertraege']?.['status'] ?? [],
  kuendigungsfrist: LOOKUP_OPTIONS['vertraege']?.['kuendigungsfrist'] ?? [],
};
const NORMALIZE_APPLOOKUPS: Record<string, string> = {
  partner: APP_IDS.PARTNER,
...
export function VertraegeDialog({ open, onClose, onSubmit, defaultValues, recordId, partnerList, ansprechpartnerList, enablePhotoScan = true, enablePhotoLocation = true }: VertraegeDialogProps) {
  const [fields, setFields] = useState<Partial<Vertraege['fields']>>({});
  ...
  // Inline-Create state for "Partner" target. The dropdown's
  // "+ Neuer …" option opens a sub-dialog; on submit we POST, add the new
  // record to the local `extraPartner` list, and select it in
  // the originating Combobox via the captured `createPartnerField`.
  const [createPartnerOpen, setCreatePartnerOpen] = useState(false);
  const [createPartnerInitial, setCreatePartnerInitial] = useState('');
  const [createPartnerField, setCreatePartnerField] = useState<string>('');
  const [extraPartner, setExtraPartner] = useState< Partner[]>([]);
  const partnerListAll = useMemo(
    () => [...partnerList, ...extraPartner],
    [partnerList, extraPartner],
  );
  function openCreatePartner(fieldKey: string, q: string) {
    setCreatePartnerField(fieldKey);
    setCreatePartnerInitial(q);
  ...
  const REQUIRED_FIELDS = ['vertragsbezeichnung', 'status', 'partner', 'startdatum'] as const;
  ...
  }), [partnerList, ansprechpartnerList, ]);
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Vertraege['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'vertraege');
      await onSubmit(clean as Vertraege['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      contextParts.push(`<available-records field="partner" entity="Partner">\n${JSON.stringify(partnerList.map(r => ({ record_id: r.record_id, ...r.fields })), null, 2)}\n</available-records>`);
      ...
      const schema = `{\n  "vertragsbezeichnung": string | null, // Vertragsbezeichnung\n  "vertragsnummer": string | null, // Vertragsnummer\n  "kategorie": LookupValue | null, // Kategorie (select one key: "dienstleistung" | "liefervertrag" | "lizenzvertrag" | "mietvertrag" | "wartungsvertrag" | "rahmenvertrag" | "kooperationsvertrag" | "sonstiges") mapping: dienstleistung=Dienstleistung, liefervertrag=Liefervertrag, lizenzvertrag=Lizenzvertrag, mietvertrag=Mietvertrag, wartungsvertrag=Wartungsvertrag, rahmenvertrag=Rahmenvertrag, kooperationsvertrag=Kooperationsvertrag, sonstiges=Sonstiges\n  "status": LookupValue | null, // Status (select one key: "aktiv" | "in_verhandlung" | "ruhend" | "gekuendigt" | "abgelaufen" | "entwurf") mapping: aktiv=Aktiv, in_verhandlung=In Verhandlung, ruhend=Ruhend, gekuendigt=Gekündigt, abgelaufen=Abgelaufen, entwurf=Entwurf\n  "partner": string | null, // Display name from Partner (see <available-records>)\n  "ansprechpartner": string | null, // Display name from Ansprechpartner (see <available-records>)\n  "jahresvolumen": number | null, // Jahresvolumen (€)\n  "startdatum": string | null, // YYYY-MM-DD\n  "enddatum": string | null, // YYYY-MM-DD\n  "kuendigungsfrist": LookupValue | null, // Kündigungsfrist (select one key: "zwei_wochen" | "ein_monat" | "zwei_monate" | "drei_monate" | "sechs_monate" | "zwoelf_monate" | "keine") mapping: zwei_wochen=2 Wochen, ein_monat=1 Monat, zwei_monate=2 Monate, drei_monate=3 Monate, sechs_monate=6 Monate, zwoelf_monate=12 Monate, keine=Keine\n  "kuendigungsdatum": string | null, // YYYY-MM-DD\n  "notizen": string | null, // Notizen\n}`;
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["partner", "ansprechpartner"]);
        for (const [k, v] of Object.entries(raw)) {
          if (applookupKeys.has(k)) continue;
          if (v != null) merged[k] = v;
        }
        const partnerName = raw['partner'] as string | null;
        if (partnerName) {
          const partnerMatch = partnerList.find(r => matchName(partnerName!, [String(r.fields.firmenname ?? '')]));
          if (partnerMatch) merged['partner'] = createRecordUrl(APP_IDS.PARTNER, partnerMatch.record_id);
        }
        ...
        return merged as Partial<Vertraege['fields']>;
      });
      ...
      if (file && (file.type.startsWith('image/') || file.type === 'application/pdf')) {
        try {
          ...
          setFields(prev => ({ ...prev, vertragsdokument: fileUrl }));
        } catch (uploadErr) {
  ...
  const DIALOG_INTENT = defaultValues ? 'Verträge bearbeiten' : 'Verträge hinzufügen';

  const fieldBlocks: Record<string, React.ReactNode> = {
    'vertragsbezeichnung': (
      <div key="vertragsbezeichnung" className="space-y-1.5">
        <Label htmlFor="vertragsbezeichnung">Vertragsbezeichnung <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="vertragsbezeichnung"
          placeholder="z. B. Lizenz Microsoft 365"
          value={fields.vertragsbezeichnung ?? ''}
          onChange={e => setFields(f => ({ ...f, vertragsbezeichnung: e.target.value }))}
          required
        />
        {showErrors && !fields.vertragsbezeichnung && (
          <p className="text-xs text-destructive mt-1">Pflichtfeld</p>
        )}
      </div>
    ),
    'vertragsnummer': (
      <div key="vertragsnummer" className="space-y-1.5">
        <Label htmlFor="vertragsnummer">Vertragsnummer</Label>
        <Input
          id="vertragsnummer"
          placeholder="z. B. LZ-2026-001"
    ...
    'kategorie': (
      <div key="kategorie" className="space-y-1.5">
        ...
        >
          <SelectTrigger id="kategorie" className="max-sm:h-11"><SelectValue placeholder="z. B. Lizenzvertrag, Dienstleistung" /></SelectTrigger>
          <SelectContent>
    ...
    'status': (
      <div key="status" className="space-y-1.5">
        ...
        >
          <SelectTrigger id="status" className="max-sm:h-11"><SelectValue placeholder="z. B. Aktiv, In Verhandlung" /></SelectTrigger>
          <SelectContent>
            <SelectItem value="none">—</SelectItem>
    ...
    'partner': (
      <div key="partner" className="space-y-1.5">
        <Label htmlFor="partner">Partner <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Combobox
          id="partner"
          placeholder="Welcher Partner?"
          items={partnerListAll.map(r => ({
            id: r.record_id,
            label: String(r.fields.firmenname ?? r.record_id),
          }))}
          value={extractRecordId(fields.partner)}
          onChange={id => setFields(f => ({ ...f, partner: id ? createRecordUrl(APP_IDS.PARTNER, id) : undefined }))}
          searchPlaceholder="Suchen…"
          emptyText="Kein Treffer"
          onCreateNew={(q) => openCreatePartner("partner", q)}
          createLabel="Neu in Partner"
        />
        {showErrors && !fields.partner && (
          <p className="text-xs text-destructive mt-1">Pflichtfeld</p>
        )}
      </div>
    ),
    'ansprechpartner': (
      <div key="ansprechpartner" className="space-y-1.5">
        ...
        <Combobox
          ...
          createLabel="Neu in Ansprechpartner"
        />
      </div>
    ),
    'jahresvolumen': (
      <div key="jahresvolumen" className="space-y-1.5">
        <Label htmlFor="jahresvolumen">Jahresvolumen (€)</Label>
        <Input
          id="jahresvolumen"
          type="number"
          step="any"
          {...numberInputProps(formEnhancements, 'jahresvolumen')}
          placeholder="z. B. 12500,00"
          value={fields.jahresvolumen !== undefined ? fields.jahresvolumen : (computedValues['jahresvolumen'] ?? '')}
          onChange={e => setFields(f => ({ ...f, jahresvolumen: clampNumberValue(formEnhancements, 'jahresvolumen', e.target.value) }))}
        />
      </div>
    ),
    'startdatum': (
      <div key="startdatum" className="space-y-1.5">
        ...
        <DatePicker
          id="startdatum"
          placeholder="Wann beginnt der Vertrag?"
          mode="date"
    ...
    'enddatum': (
      <div key="enddatum" className="space-y-1.5">
        ...
        <DatePicker
          id="enddatum"
          placeholder="Wann endet der Vertrag?"
          mode="date"
    ...
    'kuendigungsfrist': (
      <div key="kuendigungsfrist" className="space-y-1.5">
        ...
        >
          <SelectTrigger id="kuendigungsfrist" className="max-sm:h-11"><SelectValue placeholder="z. B. 3 Monate, 1 Monat" /></SelectTrigger>
          <SelectContent>
            <SelectItem value="none">—</SelectItem>
    ...
    'kuendigungsdatum': (
      <div key="kuendigungsdatum" className="space-y-1.5">
        <Label htmlFor="kuendigungsdatum">Kündigungsdatum</Label>
        <DatePicker
          id="kuendigungsdatum"
          placeholder="Wann wurde gekündigt?"
          mode="date"
    ...
    'vertragsdokument': (
      <div key="vertragsdokument" className="space-y-1.5">
        <Label htmlFor="vertragsdokument">Vertragsdokument</Label>
        {fields.vertragsdokument ? (
          <div className="flex items-center gap-3 rounded-lg border p-2">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm truncate text-foreground">{fields.vertragsdokument.split("/").pop()}</p>
              <div className="flex gap-2 mt-1">
                  ...
                  <input
                    ...
                    onChange={async (e) => {
                      ...
                      try {
                        ...
                        setFields(f => ({ ...f, vertragsdokument: fileUrl }));
                ...
                <button
                  ...
                  onClick={() => setFields(f => ({ ...f, vertragsdokument: undefined }))}
                >
                  Entfernen
                </button>
              </div>
    ...
    'notizen': (
      <div key="notizen" className="space-y-1.5">
        ...
        <Textarea
          id="notizen"
          placeholder="Besonderheiten, Laufzeitanpassungen, Rabatte..."
  ...
  const FIELD_LABELS: Record<string, string> = {"vertragsbezeichnung": "Vertragsbezeichnung", "vertragsnummer": "Vertragsnummer", "kategorie": "Kategorie", "status": "Status", "partner": "Partner", "ansprechpartner": "Ansprechpartner", "jahresvolumen": "Jahresvolumen (€)", "startdatum": "Startdatum", "enddatum": "Enddatum", "kuendigungsfrist": "Kündigungsfrist", "kuendigungsdatum": "Kündigungsdatum", "vertragsdokument": "Vertragsdokument", "notizen": "Notizen"};
  const CURRENCY_KEYS = new Set<string>(["jahresvolumen"]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"partner": {"firmenname": "Firmenname", "branche": "Branche", "strasse": "Straße", "hausnummer": "Hausnummer", "postleitzahl": "Postleitzahl", "ort": "Ort", "telefon": "Telefon", "email": "E-Mail", "webseite": "Webseite", "notizen": "Notizen"}, "ansprechpartner": {"vorname": "Vorname", "nachname": "Nachname", "position": "Position", "email_ap": "E-Mail", "telefon_ap": "Telefon", "partner": "Partner"}};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.VERTRAEGE} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
    ...
    {createPartnerOpen && (
      <PartnerDialog
        ...
        onSubmit={async (newFields) => {
          const result = await LivingAppsService.createPartnerEntry(newFields as any) as { id?: string };
          if (result?.id) {
            const newRec = { record_id: result.id, fields: newFields } as unknown as Partner;
            setExtraPartner(prev => [...prev, newRec]);
            const url = createRecordUrl(APP_IDS.PARTNER, result.id);
            setFields(prev => ({ ...prev, [createPartnerField]: url } as any));
          }
          setCreatePartnerOpen(false);
        }}
        defaultValues={createPartnerInitial
          ? ({ firmenname: createPartnerInitial } as any)
          : undefined}
      />
    )}
