...
import type { Vertraege, Partner, Ansprechpartner, CreateVertraege, CreatePartner, CreateAnsprechpartner } from '@/types/app';
...
export class LivingAppsService {
  // --- VERTRAEGE ---
  static async getVertraege(): Promise<Vertraege[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.VERTRAEGE}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Vertraege[];
    return enrichLookupFields(records, 'vertraege');
  }
  static async getVertraegeEntry(id: string): Promise<Vertraege | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.VERTRAEGE}/records/${id}`);
    const record = { record_id: data.id, ...data } as Vertraege;
    return enrichLookupFields([record], 'vertraege')[0];
  }
  static async createVertraegeEntry(fields: CreateVertraege) {
    return callApi('POST', `/apps/${APP_IDS.VERTRAEGE}/records`, { fields: cleanFieldsForApi(fields as any, 'vertraege') });
  }
  static async updateVertraegeEntry(id: string, fields: Partial<CreateVertraege>) {
    return callApi('PATCH', `/apps/${APP_IDS.VERTRAEGE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'vertraege') });
  }
  static async deleteVertraegeEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.VERTRAEGE}/records/${id}`);
  }

  // --- PARTNER ---
  static async getPartner(): Promise<Partner[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.PARTNER}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Partner[];
    return enrichLookupFields(records, 'partner');
  }
  static async getPartnerEntry(id: string): Promise<Partner | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.PARTNER}/records/${id}`);
    const record = { record_id: data.id, ...data } as Partner;
    return enrichLookupFields([record], 'partner')[0];
  }
  static async createPartnerEntry(fields: CreatePartner) {
    return callApi('POST', `/apps/${APP_IDS.PARTNER}/records`, { fields: cleanFieldsForApi(fields as any, 'partner') });
  }
  static async updatePartnerEntry(id: string, fields: Partial<CreatePartner>) {
    return callApi('PATCH', `/apps/${APP_IDS.PARTNER}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'partner') });
  }
  static async deletePartnerEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.PARTNER}/records/${id}`);
  }

  // --- ANSPRECHPARTNER ---
  static async getAnsprechpartner(): Promise<Ansprechpartner[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.ANSPRECHPARTNER}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Ansprechpartner[];
    return enrichLookupFields(records, 'ansprechpartner');
  }
  static async getAnsprechpartnerEntry(id: string): Promise<Ansprechpartner | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.ANSPRECHPARTNER}/records/${id}`);
    const record = { record_id: data.id, ...data } as Ansprechpartner;
    return enrichLookupFields([record], 'ansprechpartner')[0];
  }
  static async createAnsprechpartnerEntry(fields: CreateAnsprechpartner) {
    return callApi('POST', `/apps/${APP_IDS.ANSPRECHPARTNER}/records`, { fields: cleanFieldsForApi(fields as any, 'ansprechpartner') });
  }
  static async updateAnsprechpartnerEntry(id: string, fields: Partial<CreateAnsprechpartner>) {
    return callApi('PATCH', `/apps/${APP_IDS.ANSPRECHPARTNER}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'ansprechpartner') });
  }
  static async deleteAnsprechpartnerEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.ANSPRECHPARTNER}/records/${id}`);
  }
